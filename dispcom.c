...
void DispCom_Config_Rx_Interrupt(void){
  USART1->CR1  |= USART_CR1_RXNEIE_RXFNEIE;
	...
	NVIC_SetPriority(USART1_IRQn, DISPCOM_INT_PRIORITY);
}
...
void DispCom_Buf_Set(uint16_t index, uint8_t val){
  DispCom.Buf[index] = val;
}
...
void DispCom_Data_Print_Buf(void){
  for(uint16_t i = 0; i < DispCom_Data_Len_Get(); i++){
	  //DispCom_Tx_Byte( DispCom_Buf_Get(i) );
		//Debug_Tx_Byte( DispCom_Buf_Get(i) );
  }
  //DispCom_Tx_NL();
	//Debug_Tx_NL();
}
